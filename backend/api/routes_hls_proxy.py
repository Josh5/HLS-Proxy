...
hls_proxy_prefix = os.environ.get('HLS_PROXY_PREFIX', 'proxy')
...
def add_proxy_arg(url, base_url):
    ...
    return f'{hls_proxy_prefix}.{extension}?encoded_remote={full_url_encoded}'
...
async def cache_url_file(url, headers):
    ...
    try:
        ...
        current_app.logger.warning(f"Failed to cache URL '%s' to cache: %s - %s", url, cache_key, e)
...
@blueprint.route(f'/{hls_proxy_prefix}.m3u8', methods=['GET'])
async def hls_proxy_m3u8():
    ...
    current_app.logger.info(f"[PROXIED] Serving URL '%s' without cache", upstream_url)
    ...
    async with aiohttp.ClientSession() as session:
        async with session.request(
            ...
            for line in lines:
                if line.startswith("#EXT-X-KEY"):
                    ...
                    if key_uri:
                        ...
                        updated_line = line.replace(key_uri, f'{hls_proxy_prefix}.key?key_cache_id={key_cache_id}')
            ...
            # Create a background task to clean up old cached items
            current_app.add_background_task(evict_cache_task)

            return response


@blueprint.route(f'/{hls_proxy_prefix}.key', methods=['GET'])
async def hls_proxy_key():
    ...
    if await cache.exists(key_cache_id):
        current_app.logger.info(f"[CACHED] Serving key URL from cache: %s", key_cache_id)
        return await send_from_cache(key_cache_id, content_type="binary/octet-stream")
    current_app.logger.error(f"The key requires a cached path to serve.")
    return Response(f"The key requires a cached path to serve.", status=400)


@blueprint.route(f'/{hls_proxy_prefix}.ts', methods=['GET'])
async def hls_proxy_ts():
    ...
    if await cache.exists(cache_key):
        current_app.logger.info(f"[CACHED] Serving URL '%s' from cache: %s", upstream_url, cache_key)
